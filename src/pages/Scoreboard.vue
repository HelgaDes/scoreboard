...
async function load(){
  const res = await getScoreService().getScoreboard({ tab: activeTab.value as any, limit: 7, sort: 'monthly', order: 'desc' })
...
async function refreshGroups(){ groups.value = await getGroupService().list() }
refreshGroups()
...
function toggleSelect(id:string|number){ const s = new Set(selectedIds.value); s.has(id) ? s.delete(id) : s.add(id); selectedIds.value = Array.from(s) }
...
async function openGroupingModal(){
  ...
  const name = window.prompt('Name group') // minimal without extra component; can be replaced by modal
  if(!name) return
  await getGroupService().create({ name, memberAgentIds: [...selectedIds.value], tab: activeTab.value as any })
  selectedIds.value = []
  await refreshGroups()
}
async function ungroup(id:string){ await getGroupService().remove(id); await refreshGroups() }
</script>

<template>
  <HeaderBar :show-actions="true" :volume-on="sound.enabled" @toggle-volume="sound.toggleEnabled()" @open-melody="showMelody=true" @logout="$router.push('/login')" />

  <section class="table-section" style="width:960px;padding:15px 70px 36px 70px;display:flex;justify-content:center;align-items:center;">
    <TableContainer :width="820" :overlay-top="-20">
      <template #overlay><TotalWidget :value="fmt(totalMonthly)" /></template>
      ...
      <template #header>
        ...
        <div style="display:inline-grid;grid-auto-flow:column;column-gap:8px;align-items:center;">
          ...
          <ButtonSecondary :pressed="groupingOn" @click="toggleGrouping" />
        </div>
      </template>

      <ScoreTable :rows="visibleRows" :grouping-on="groupingOn" :selected-ids="selectedIds" @toggle-select="toggleSelect" @ungroup="ungroup" />
      <template #totals><TotalsRow :rows="visibleRows as any" /></template>
    </TableContainer>
  </section>

  <teleport to="#stage-overlay">
    <div v-if="showMelody" style="position:fixed;inset:0;display:grid;place-items:center;background:rgba(0,0,0,.2)">
      <SelectMelodyModal :onClose="()=>showMelody=false"/>
    </div>
  </teleport>
</template>
