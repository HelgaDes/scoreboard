...
import type { GroupDTO, UpsertGroupInput } from './types'
...
export function createHttpGroupService(baseUrl: string): GroupService {
  ...
  return {
    async list(){ return http<GroupDTO[]>(api('/groups')) },
    async create(input){ return http<GroupDTO>(api('/groups'), { method:'POST', body: JSON.stringify(input) }) },
    async remove(id){
      try { await http<void>(api(`/groups/${id}`), { method: 'DELETE' }) }
      catch (e) { if (e instanceof HttpError) throw new Error(`Group delete ${e.status}`); throw e }
    },
  }
}
...
export function createMockGroupService(): GroupService {
  ...
  return {
    ...
    async create(input){
      ...
      const g: GroupDTO = { id, name: input.name, memberAgentIds: [...input.memberAgentIds], tab: input.tab }
      store.set(id, g)
      return g
    },
